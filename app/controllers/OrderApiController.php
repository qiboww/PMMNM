...
class OrderApiController {
    ...
    public function show($id) {
        ...
        // Kiểm tra quyền sở hữu đơn hàng (chỉ Admin hoặc chính chủ được xem)
        if ($currentUser['role'] !== 'admin' && $order['user_id'] != $currentUser['id']) {
            http_response_code(403);
            echo json_encode(["status" => "error", "message" => "Forbidden: Bạn không có quyền xem đơn hàng này"]);
            return;
        }
    ...
    public function destroy($id) {
        ...
        // Kiểm tra quyền sở hữu đơn hàng (chỉ chính chủ hoặc Admin được hủy)
        $stmtCheck = $this->conn->prepare("SELECT user_id, status FROM orders WHERE id = :id");
        ...
        if ($order['status'] !== 'pending') {
            http_response_code(400);
            echo json_encode(["status" => "error", "message" => "Chỉ có thể hủy đơn hàng ở trạng thái chờ xử lý (pending)"]);
            return;
        }

        // Chỉ cho phép hủy bằng cách chuyển status thành 'canceled' để giữ lịch sử, không xóa cứng trong database.
        $stmt = $this->conn->prepare("UPDATE orders SET status = 'canceled' WHERE id = :id");
        $stmt->execute([':id' => $id]);
