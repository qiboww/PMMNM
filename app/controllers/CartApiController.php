...
class CartApiController {
    ...
    public function store() {
        ...
        $stmtCheck->execute([':user_id' => $data->user_id, ':product_id' => $data->product_id]);
    ...
    // DELETE: /api/cart/{cart_id} HOẶC /api/cart/clear?user_id={id}
    public function destroy($id) {
        ...
        if ($id === 'clear') {
            ...
            if (!$user_id) {
                http_response_code(400);
                echo json_encode(["status" => "error", "message" => "Thiếu user_id để xóa giỏ hàng"]);
                return;
            }
